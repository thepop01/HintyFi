...
const ProjectLeaderboardPage: React.FC = () => {
    ...
    const [projectCampaigns, setProjectCampaigns] = useState<CampaignEvent[]>([]);

    useEffect(() => {
        const fetchData = async () => {
            if (!id) return;
            const projects = await getProjects();
            const projectData = projects.find(p => p.id === id) || projects.find(p => p.name.toLowerCase() === id?.toLowerCase());
            ...
            if (projectData) {
                ...
                const campaignEvents = eventsData.filter((e): e is CampaignEvent => e.type === 'campaign' && e.projectName === projectData?.name);
                setProjectCampaigns(campaignEvents);
            }
        };
        fetchData();
    }, [id]);
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6">
            <button onClick={() => navigate(`/${project.name.toLowerCase()}`)} className="neu-button px-4 py-2 flex items-center gap-2 w-max">
                <ArrowLeft size={16} />
                Back to Project
            </button>
                    ...
                    {paginatedLeaderboard.map((item, index) => {
                        ...
                        <motion.li key={item.user.id} layout>
                            ...
                            >
                                ...
                                <img 
                                    src={item.user.profile_pic_url || `https://i.pravatar.cc/40?u=${item.user.id}`}
                                    alt={item.user.name} 
                                    className="w-10 h-10 rounded-full object-cover border-2 border-surface" 
                                />
                                <ReactRouterDOM.Link to={`/profile/${item.user.id}`} onClick={e=>e.stopPropagation()} className="font-semibold text-on-surface hover:text-primary transition-colors flex-grow truncate flex items-center gap-2">
                                    {item.user.name}
