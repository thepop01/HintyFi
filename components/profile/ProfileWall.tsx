...
const ProfileWall: React.FC<ProfileWallProps> = ({ user, isOwnProfile, onEdit }) => {
    const {
        id,
        name,
        profilePic,
        ...
        <div className="profile-wall-container -mx-4 sm:-mx-6 lg:-mx-8">
            <div className="profile-content-wrapper">
                <div className="profile-badge relative">
                    {isOwnProfile && (
                        <button onClick={onEdit} className="absolute top-2 right-2 neu-button !rounded-full !w-8 !h-8 p-0 flex items-center justify-center" title="Edit Profile">
                            <Edit size={14} />
                        </button>
                    )}
                    ...
                    <div className="flex items-center justify-center gap-3">
                        ...
                        {user.socials?.discord && (
                            <button onClick={() => handleCopy(user.socials!.discord!, 'Discord username copied!')} title="Copy Discord ID" className="neu-button !rounded-full !w-10 !h-10 flex items-center justify-center p-0">
                                <DiscordIcon size={18} />
                            </button>
                        )}
                        {user.socials?.github && (
                            <a href={`https://github.com/${user.socials.github}`} target="_blank" rel="noopener noreferrer" title="View on GitHub" className="neu-button !rounded-full !w-10 !h-10 flex items-center justify-center p-0">
                                <GitHubIcon size={18} />
                            </a>
                        )}
                        {user.walletAddress && (
                            <button onClick={() => handleCopy(user.walletAddress!, 'Wallet address copied!')} title="Copy Wallet Address" className="neu-button !rounded-full !w-10 !h-10 flex items-center justify-center p-0">
                                <Wallet size={18} />
                            </button>
                        )}
                    </div>
