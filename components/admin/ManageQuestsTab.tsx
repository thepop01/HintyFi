...
const ManageQuestsTab: React.FC = () => {
    ...
    const allQuests = useMemo(() => getQuests(), [dataVersion]);
    ...
    const { ongoingQuests, pastQuests } = useMemo(() => {
        const displayQuests: DisplayQuest[] = allQuests.map(q => {
            ...
            if (q.category === 'Identity' && q.identityQuestion) {
                type = 'identity';
    ...
    const handleEdit = (displayQuest: DisplayQuest) => {
        if (editingQuestId === displayQuest.id) {
            setEditingQuestId(null);
            setEditingQuest(null);
            return;
        }
        const originalQuest = getQuestById(displayQuest.originalId);
        if (originalQuest) {
    ...
    const handleSave = (questDataFromForm: Partial<Quest>) => {
        ...
        if (isNew) {
            ...
            const originalQuest = getQuestById(questDataFromForm.id!);
            if (!originalQuest) {
        ...
        if (addOrUpdateQuest(questToSave)) {
