...
const IDOPage: React.FC = () => {
    ...
    const { allProjects, settings } = useMemo(() => {
        ...
        return {
            ...
            settings: getSiteContentSettings().ido,
        };
    }, []);
    
    // FIX: Simplified isPublished check for better readability.
    const launchpadProjects = useMemo(() => allProjects.filter(p => p.category.includes('launchpad') && p.isPublished), [allProjects]);
        ...
        <div className="max-w-[84rem] mx-auto">
            ...
            {paginatedProjects.length > 0 ? (
                ...
                <EmptyState
                    icon={<Rocket size={48} />}
                    title="No IDOs Found"
                    description="Your search and filter combination did not return any results. Check back soon for new launches!"
                />
            )}
        </div>
    );
