                ...
                <tbody>
                    {isLoading ? (
                        ...
                        collections.map((collection, index) => {
                            ...
                            const sentimentScore = project && project.votes ? project.votes.up - project.votes.down : 0;
                            ...
                            return (
                                <React.Fragment key={collection.id}>
                                    <tr className="hover:bg-border/10 transition-colors cursor-pointer border-b border-white" onClick={() => onRowClick(collection.id)}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <ReactRouterDOM.Link
                                                        to={`/${project ? project.name.toLowerCase() : collection.projectId}`}
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {collection.perksIn && collection.perksIn.length > 0 ? (
                                                <div className="flex items-center -space-x-2">
                                                    {collection.perksIn.slice(0, 2).map(p => (
                                                        <ReactRouterDOM.Link to={`/${p.projectName.toLowerCase()}`} key={p.projectId} onClick={e => e.stopPropagation()}>
                                        ...
                                        {isExpanded && project && (
                                            <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                                <td colSpan={10} className="p-0 bg-[#ddd7fe]">
                                                    <ProjectProfileInline project={project} collection={collection as any} />
                                                </td>
                                            </motion.tr>
                                        )}
                                    </AnimatePresence>
...
const NFTPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        getSiteContentSettings().then(s => setSettings(s.nft));
    }, []);
            ...
            filtered.sort((a, b) => {
                ...
                switch (sortConfig.key) {
                    ...
                    case 'sentiment':
                        ...
                        aValue = projectA && projectA.votes ? projectA.votes.up - projectA.votes.down : -Infinity;
                        bValue = projectB && projectB.votes ? projectB.votes.up - projectB.votes.down : -Infinity;
                        break;
                    case 'perksIn': aValue = a.perksIn ? a.perksIn.length : 0; bValue = b.perksIn ? b.perksIn.length : 0; break;
