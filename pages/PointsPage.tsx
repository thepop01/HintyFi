...
import { User, Project, Event, Quest, CampaignEvent, PointTier, ManualHintAdjustment } from '../src/types';
...
// Component for Hint Points breakdown
const HintPointsTab: React.FC<{ user: User; projects: Project[], events: Event[]; quests: Quest[] }> = ({ user, projects, events, quests }) => {
    const { isTaskCompleted } = useCompletedTasks();
    
    const { hintBreakdown, totalHint, rewardsAdjustments } = useMemo(() => {
        const breakdown = {
            projectVotes: 0,
        ...
        if (user.discordRoles?.some(r => r.name === 'with full access')) {
            breakdown.projectVotes = (user.votedProjectIds || []).length;
        }

        const userRoles = new Set(user.discordRoles?.map(r => r.roleId) || []);
        ...
        projects.forEach(project => {
            project.discordRoles?.forEach(role => {
                if (role.roleId && userRoles.has(role.roleId) && role.points) {
        ...
        // --- MANUAL ADJUSTMENTS ---
        const manualRewardsAdjustments: ManualHintAdjustment[] = [];
        (user.manualHintAdjustments || []).forEach(adj => {
        ...
        const totalHint = Object.values(breakdown)
            .map(categoryValue => {
        ...
        return { hintBreakdown: breakdown, totalHint: Math.round(totalHint), rewardsAdjustments: manualRewardsAdjustments };
    }, [user, projects, events, quests, isTaskCompleted]);
    
    return (
        <div className="space-y-6">
            <div className="neu-outset-card p-4 text-center">
                <p className="text-sm font-semibold text-on-surface-variant">Total Hint Points</p>
                <p className="text-4xl font-bold text-accent">{totalHint.toLocaleString()}</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                    <h3 className="font-bold text-lg text-on-surface-variant">Engagement</h3>
                    <PointEntry icon={<Vote />} label="Project Votes" points={hintBreakdown.projectVotes} subtext={`${hintBreakdown.projectVotes} vote(s)`} />
                    <PointEntry icon={<Vote />} label="Campaign Votes" points={hintBreakdown.campaignVotes} subtext={`${hintBreakdown.campaignVotes} vote(s)`} />
                    <PointEntry icon={<FilePen />} label="Campaign Submissions" points={hintBreakdown.campaignSubmissions} subtext={`${hintBreakdown.campaignSubmissions} submission(s)`} />
                    <PointEntry icon={<FilePen />} label="Quest Answers" points={hintBreakdown.questAnswers} subtext={`${hintBreakdown.questAnswers} answer(s)`} />
                    <PointEntry icon={<Trophy />} label="Campaign Wins" points={hintBreakdown.campaignWins} subtext={`${hintBreakdown.campaignWins > 0 ? 'From wins & adjustments' : ''}`} />
                </div>
                 <div className="space-y-4">
                    <h3 className="font-bold text-lg text-on-surface-variant">Contributions</h3>
                    <PointEntry icon={<ClipboardCheck />} label="Task Completion" points={hintBreakdown.tasks.points} subtext={`from ${hintBreakdown.tasks.count} task(s)`} />
             ...
             <div>
                <h3 className="font-bold text-lg text-on-surface-variant mb-2">Holding & Roles</h3>
                 <div className="space-y-2">
                    <PointEntry icon={<Shield />} label="Discord Roles" points={hintBreakdown.roles.points} subtext={`from ${hintBreakdown.roles.count} role(s)`} />
                    <PointEntry icon={<Gem />} label="NFT Holding" points={Math.round(hintBreakdown.nfts.points)} subtext={`from ${hintBreakdown.nfts.count} collection(s)`} />
                    <PointEntry icon={<Banknote />} label="Token Holding" points={Math.round(hintBreakdown.tokens.points)} subtext={`from ${hintBreakdown.tokens.count} token type(s)`} />
                </div>
            </div>
        </div>
    );
...
const PointsPage: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'credo' | 'hint'>('hint');
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <button onClick={() => navigate(`/ledger/${user.id}`)} className="neu-button px-4 py-2 flex items-center gap-2">
                <ArrowLeft size={16} /> Back to Profile
            </button>
            <div className="neu-card p-6 flex flex-col sm:flex-row items-center gap-6">
                <img src={user.profile_pic_url || `https://i.pravatar.cc/150?u=${user.id}`} alt={user.name} className="w-24 h-24 rounded-full border-4 border-surface bg-background object-contain p-1" />
            ...
            <div className="neu-card p-2 sm:p-4">
                  <div className="flex border-b border-border/10 mb-4 gap-4">
                     <button onClick={() => setActiveTab('hint')} className={`neu-tab-button ${activeTab === 'hint' ? 'active' : ''}`}><Star className="inline mr-2" size={16} /> Hint Points</button>
                 ...
                 <AnimatePresence mode="wait">
                    ...
                    >
                        {activeTab === 'credo' ? <CredoPointsTab user={user} projects={projects} /> : <HintPointsTab user={user} projects={projects} events={events} quests={quests} />}
                    </motion.div>
                </AnimatePresence>
            </div>
        </div>
