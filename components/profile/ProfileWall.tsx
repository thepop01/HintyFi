...
import { Wallet, ShieldCheck, BarChart, Edit } from 'lucide-react';
...
const ProfileWall: React.FC<ProfileWallProps> = ({ user, isOwnProfile }) => {
    const {
        id,
        name,
        profile_pic_url,
    ...
    const credoPoints = useMemo(() => {
        ...
        const isBuilder = projectsBuilding && projectsBuilding.length > 0;
    ...
    const userHoldings = useMemo(() => {
        if (!nftHoldings || nftHoldings.length === 0) {
            return [];
        }
    ...
    const isTeamMember = useMemo(() => {
        ...
        return allProjects.some(p => p.team?.some(t => t.name.toLowerCase() === currentUser.name.toLowerCase()));
    }, [currentUser, allProjects]);

    const canVouch = currentUser && currentUser.id !== user.id && isTeamMember;
    const hasVouched = vouchedFor?.includes(currentUser?.id || '');
    const vouchButtonDisabled = !canVouch || hasVouched || justVouched;

    const handleVouch = async () => {
        if (!currentUser || vouchButtonDisabled) return;
        
        const result = await vouchForUser(currentUser.id, user.id);
            ...
            <div className="profile-content-wrapper">
                <div className="profile-badge relative">
                    <div className="profile-identity-block">
                        <img
                            src={profile_pic_url || `https://i.pravatar.cc/150?u=${user.id}`}
                            alt={name}
                        ...
                        <div className="profile-identity-text">
                            <h2 className="profile-badge-name text-xl">{name}</h2>
                            <p className="profile-badge-score text-xl">
                    ...
                    <div className="flex items-center justify-center gap-3">
                        {user.socials?.twitter && (
                            <a href={`https://x.com/${user.socials.twitter}`} target="_blank" rel="noopener noreferrer" title="View on X" className="neu-button !rounded-full !w-10 !h-10 flex items-center justify-center p-0">
                                <XIcon size={16} />
                            </a>
                        )}
                    ...
                    <div className="flex items-center gap-2 mt-auto">
                        {!isOwnProfile && (
                            <button
                                ...
                                className={`profile-vouch-button flex-1 ${hasVouched || justVouched ? 'vouched' : 'can-vouch'} ${!isTeamMember ? 'cursor-not-allowed' : ''}`}
                                title={!isTeamMember ? "Only team members can vouch for users." : (hasVouched || justVouched ? "You have already vouched for this user" : "Vouch for this user")}
                            >
                                <ShieldCheck size={20} />
                                <span>{hasVouched || justVouched ? 'Vouched' : 'Vouch'}</span>
                            </button>
                        )}
                        <ReactRouterDOM.Link to={`/ledger/${user.id}/points?user=${user.id}`} className="profile-vouch-button can-vouch flex-1">
                            <BarChart size={20} />
                            <span>Points</span>
                        </ReactRouterDOM.Link>
                    </div>
                        ...
                        >
                            ...
                            <ul>
                                {buildingProjects.map(project => (
                                    <li key={project.id}>
                                        <ReactRouterDOM.Link to={`/${project.name.toLowerCase()}`} className="profile-project-item group">
                        ...
                        >
                            ...
                            <ul>
                                {discordRoles.map(role => {
                                    const projectName = roleToProjectMap.get(role.name);
                                    return (
                                        <li key={role.id} className="profile-role-item">
                                            {projectName ? `${projectName} - ${role.name}` : role.name}
                                        </li>
                                    );
                                })}
                            </ul>
                            ...
                            <ul>
                                {userHoldings.map(collection => (
                                    <li key={collection.id}>
                                        <ReactRouterDOM.Link to={`/${collection.projectName.toLowerCase()}`} className="profile-project-item group">
