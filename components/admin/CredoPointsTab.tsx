...
import { getEvents, deleteItem } from '../../src/services/dataService';
import { useSuperAdminContext } from '../../context/SuperAdminContext';
import ConfirmationModal from '../common/ConfirmationModal';
import { useToast } from '../../context/ToastContext';
import Modal from '../common/Modal';

const CredoPointsTab: React.FC = () => {
    const { approvedProjects: allProjects, users, refreshData } = useSuperAdminContext();
    ...
    const projectsWithPendingCounts = useMemo(() => {
        return allProjects.map(project => {
            const pendingLinksCount = (project.hasPendingChanges && project.pendingChanges?.links && Object.keys(project.pendingChanges.links).length > 0) ? 1 : 0;
            const pendingCampaignsCount = allEvents.filter(e => e.projectName === project.name && e.status === 'pending').length;
            const pendingTasksCount = project.tasks?.filter(t => t.status === 'pending').length || 0;
            const totalPending = pendingLinksCount + pendingCampaignsCount + pendingTasksCount;
            return { ...project, totalPending };
        });
    }, [allProjects, allEvents]);

    const filteredProjects = useMemo(() => {
        if (!searchTerm) {
            return projectsWithPendingCounts;
        }
        return projectsWithPendingCounts.filter(p => fuzzySearch(searchTerm, p.name));
    }, [projectsWithPendingCounts, searchTerm]);
    ...
    const confirmDelete = () => {
        ...
        if (deleteItem('projects', itemToDelete.id)) {
            addToast(`Project "${itemToDelete.name}" deleted.`, 'success');
            refreshData();
        } else {
    ...
    return (
        <div className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <button
                    onClick={() => navigate('/super-admin/project-detail/add')}
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="font-bold text-lg text-on-surface flex items-center gap-2">
                                    {project.name}
                                    {project.isCrowned && <Crown size={16} className="text-yellow-500 fill-current" />}
                                </h2>
