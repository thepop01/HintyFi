...
import { Project, Perk, PerkType, NftPerk, NftCollection, DisplayCollection } from '../../src/types';
        ...
        <div className="neu-outset-card overflow-hidden flex flex-col h-min">
            ...
            >
                <div className="flex items-center gap-3">
                    <img src={grantingProject.logo} alt={grantingProject.name} className="w-8 h-8 rounded-full bg-surface" />
            ...
            <div className="lg:col-span-1 space-y-6">
                {showPerks && collectionsToShow.length > 0 && (
                    <div>
                        ...
                        <div className="space-y-4">
                            {collectionsToShow.map((collection) => {
                                if (!collection.perks || collection.perks.length === 0) return null;

                                return (
                                    <div key={collection.name}>
                                        <h4 className="flex items-center gap-2 font-semibold text-sm text-on-surface mb-2">
                                            <Gem size={16} />
                                        ...
                                        <div className="space-y-2">
                                            {collection.perks.map((perk, index) => {
                                                ...
                                                const isInternalRequirement = !requirement.projectName || requirement.projectName === project.name;

                                                return (
                                                    <div key={index} className="neu-outset-card p-3">
                                                        <div className="flex items-start justify-between gap-2">
                                                            <div>
                                                                <p className="font-bold text-on-surface text-sm">
                                                                    {isInternalRequirement 
                                                                        ? `Hold ${requirement.count}+ NFTs` 
                                                                        : `Requires: Hold ${requirement.count}+ of ${requirement.collectionName} from ${requirement.projectName}`
                                                                    }
                                                                </p>
                                                                <p className="text-xs text-on-surface-variant mt-1">{perk.perk.description}</p>
                                                            </div>
