...
export default function CampaignDetailPage() {
    ...
    const { campaign, project } = useMemo(() => {
        if (!id) return { campaign: null, project: null };
        const campaignData = getEvents().find(e => e.id === id && e.type === 'campaign') as CampaignEvent | undefined;
        if (!campaignData) return { campaign: null, project: null };
        const projectData = getProjects().find(p => p.name === campaignData.projectName);
        return { campaign: campaignData, project: projectData };
    }, [id, dataVersion]);

    const handleEnterCampaign = (link: string) => {
        if (!campaign || !currentUser) return;
        const { success, newEntry } = enterCampaign(campaign.id, currentUser, link);
        if (success && newEntry) {
            addToast('Your entry has been submitted!', 'success');
            setDataVersion(v => v + 1); // Refresh data
        } else {
            addToast('Failed to submit entry. You may have already entered.', 'error');
        }
    };
    
    const handleVote = (entryId: string, vote: 'up' | 'down') => {
        if (!campaign || !currentUser) {
            addToast('Please connect your wallet to vote.', 'info');
            return;
        }
        const success = voteCampaignEntry(campaign.id, entryId, currentUser, vote);
        if (success) {
            setDataVersion(v => v + 1); // Refresh data
        } else {
                    ...
                    <div className="p-6">
                     ...
                     {project && (
                         <div className="p-6">
                            ...
                            <ReactRouterDOM.Link to={`/project/${project.id}`} className="mt-2 font-semibold text-gray-700 hover:text-primary transition-colors flex items-center gap-2">
                        ...
                        >
                            {activeTab === 'submissions' ? (
                                <div className="space-y-6">
                                    ...
                                    <GiveawayDetailView campaign={campaign} onVote={handleVote} />
                                </div>
                            ) : (
