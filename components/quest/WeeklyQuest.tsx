...
const WeeklyQuest: React.FC<WeeklyQuestProps> = ({ quest, onSubmit, onVote, onSubmitIdentityAnswer, onSubmitMultipleChoiceAnswer, isDetailPage = false }) => {
  ...
  const userHasEnteredMax = currentUser ? quest.entries.filter(e => e.userId === currentUser.id).length >= quest.maxSubmissionsPerUser : false;
                    ...
                    <div className="max-h-60 overflow-y-auto space-y-2 pr-2">
                        {quest.identityQuestion.answers.map(ans => (
                            <div key={ans.id} className="p-3 bg-surface rounded-md">
                                <ReactRouterDOM.Link to={`/profile?user=${ans.userId}`} className="font-bold text-on-surface hover:underline">{ans.username}</ReactRouterDOM.Link>
