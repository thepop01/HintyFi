...
const AdminProjectDiscordRolesPage: React.FC = () => {
    ...
    const handleSaveRole = (roleData: Partial<DiscordRole>) => {
        ...
        let newRoles;
        if (isNew) {
            newRoles = [...roles, { ...roleData, roleId: roleData.roleId || uid() } as DiscordRole];
        } else {
             newRoles = roles.map(r => r.roleId === (editingRole || roleData).roleId ? { ...r, ...roleData } : r);
        }
    ...
    const handleEditClick = (role: DiscordRole) => {
        const newEditingRoleId = editingRoleId === role.roleId ? null : role.roleId;
    ...
    const confirmDelete = () => {
        ...
        const newRoles = roles.filter(r => r.roleId !== roleToDelete.roleId);
        const updatedProject = { ...project, discordRoles: newRoles };
        const success = addOrUpdateProject(updatedProject);
        ...
        <div className="space-y-6">
            ...
            <div className="neu-card p-4 space-y-3">
                {roles.length > 0 ? roles.map(role => (
                    <div key={role.roleId} className="neu-outset-card overflow-hidden">
