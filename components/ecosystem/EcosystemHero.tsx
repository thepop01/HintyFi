...
import { ArrowLeft, ArrowRight, Dna, Gem, Gamepad2, Users, Repeat, Building, Layers, Wallet, Cpu, BrainCircuit, Flame, Rocket } from 'lucide-react';
...
interface EcosystemHeroProps {
  ...
  activeCategory: ProjectCategory | 'all' | 'new' | 'hot' | 'crowned';
}
...
const EcosystemHero: React.FC<EcosystemHeroProps> = ({ projects, activeCategory }) => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (activeCategory === 'new') {
        ...
        return projects.filter(p => p.createdAt > thirtyDaysAgo);
    }
    if (activeCategory === 'crowned') {
        return projects.filter(isCrowned);
    }
    if (activeCategory === 'hot') {
        return projects.filter(p => p.isHot);
    }
    ...
    <div className="w-full">
      ...
      <div className="hidden md:block relative w-full h-[60vh] min-h-[500px]">
        ...
        <AnimatePresence>
          {hoveredProject && (
            <motion.div
              className="absolute left-[-0.5%] top-1/2 -translate-y-1/2 z-10 text-on-background pointer-events-none max-w-xs"
        ...
        <motion.div
          className="absolute -left-[15%] top-8 h-[100%] w-auto select-none z-0"
        ...
        <div className="absolute top-0 right-0 w-[85%] h-full">
            {/* Navigation Arrows */}
            {totalPages > 1 && (
                <div className="absolute top-[56%] -translate-y-1/2 left-[13%] z-30 flex flex-row gap-4">
                    <motion.button 
                        onClick={handlePrev} 
            ...
            <AnimatePresence>
                {desktopCollageProjects.map((project, index) => {
                    ...
                    return (
                        ...
                        >
                        <ReactRouterDOM.Link to={`/project/${project.id}`} className="block w-full h-full">
                            <img
                                src={project.logo}
                                alt={project.name}
