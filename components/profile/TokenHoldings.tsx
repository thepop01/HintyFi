...
const TokenPointCard: React.FC<{
    ...
    return (
        <Link to={`/${project.name.toLowerCase()}`} className="token-card">
...
const TokenHoldings: React.FC<TokenHoldingsProps> = ({ user }) => {
    ...
    const [tokenData, setTokenData] = useState<{ heldTokens: Project[], unheldTokens: Project[] }>({ heldTokens: [], unheldTokens: [] });
    ...
    useEffect(() => {
        const fetchTokenData = async () => {
            ...
            setTokenData({ heldTokens: held, unheldTokens: unheld });
        };

        fetchTokenData();
    }, [user.tokenHoldings, refreshKey]);

    const { heldTokens, unheldTokens } = tokenData;
