...
const ProjectPointsSettingsTab: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProject = async () => {
            if (initialProject.id) {
                const projectData = await getProjectById(initialProject.id);
                setFormData(projectData || null);
            }
        };
        fetchProject();
    }, [initialProject.id]);
    ...
    const toggleCrowned = () => {
        if (!formData) return;
        const updatedProject = { ...formData, is_crowned: !formData.is_crowned };
        if (addOrUpdateProject(updatedProject)) {
            addToast(`${formData.name} has been ${updatedProject.is_crowned ? 'crowned' : 'uncrowned'}.`, 'success');
    ...
    const addRole = () => {
        setFormData(prev => {
            if (!prev) return null;
            const newRole: DiscordRole = { id: uid(), serverId: '', roleId: '', name: '', description: '', points: 0 };
    ...
    const addCollection = () => {
        setFormData(prev => {
            ...
            const newCollection: NftCollection = { id: uid(), name: '', image: '', contractAddress: '', link: '', network: 'mainnet', perks: [], status: 'draft', pointsPerDay: 0 };
        ...
        <form onSubmit={handleSubmit} className="neu-card p-6">
            <div className="flex items-center justify-between gap-4 mb-6">
                ...
                <button type="button" onClick={toggleCrowned} className={`neu-button text-sm px-4 py-2 flex items-center gap-2 ${formData.is_crowned ? 'active' : ''}`}>
                    <Crown size={16} />
                    {formData.is_crowned ? 'Un-crown' : 'Crown Project'}
                </button>
            </div>
