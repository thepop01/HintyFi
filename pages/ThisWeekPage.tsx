...
const ThisWeekPage: React.FC = () => {
    ...
    const [projectsByName, setProjectsByName] = useState<Map<string, Project>>(new Map());
    ...
    useEffect(() => {
        const fetchData = async () => {
            const [projectsData, eventsData] = await Promise.all([
                getProjects(),
                getWeeklyDiscordEvents()
            ]);

            const projectMap = new Map<string, Project>();
            if (Array.isArray(projectsData)) {
                projectsData.forEach(p => { projectMap.set(p.name, p); });
            }
            setProjectsByName(projectMap);
    ...
    const weekData = useMemo(() => {
        ...
        events.forEach(event => {
            ...
            if (diffDays >= 0 && diffDays < 7) {
                const project = projectsByName.get(event.serverName);
        ...
        return days;
    }, [projectsByName, events, weekOffset]);
