...
import { Project, User, TeamMember, NftCollection, DiscordRole, PerkType, NftPerk } from '../../src/types';
interface LinkItem {
    label: string;
    url: string;
}
...
const TeamPanel: React.FC<{ team: TeamMember[]; allUsers: User[] }> = ({ team, allUsers }) => {
    ...
    const calculateCredibilityScore = (user: User): number => {
        // Team members are eligible for credo points, so display their manual score.
        ...
        <Section icon={<Users size={24} />} title="Team">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {team.map((member, index) => {
                    ...
                    const photo = member.photoUrl || user?.profile_pic_url || `https://i.pravatar.cc/150?u=${member.name}`;
                    ...
                    return (
                        <div key={index} className="neu-outset-card p-3 flex items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                                {member.twitterUrl && (
                                    <a href={member.twitterUrl} target="_blank" rel="noopener noreferrer" title="Twitter" className="neu-control !p-1.5"><XIconInline className="w-3.5 h-3.5" /></a>
                                )}
                                {member.linkedinUrl && (
                                    <a href={member.linkedinUrl} target="_blank" rel="noopener noreferrer" title="LinkedIn" className="neu-control !p-1.5"><Linkedin size={14} /></a>
                                )}
                            </div>
                        </div>
                    );
                    ...
                    >
                        <div className="pt-2 pb-4 px-4">
                            <div className="space-y-3">
                                {roles.map((role) => (
                                    <div key={role.id} className="neu-outset-card p-4 rounded-xl">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {isExternalCollection && collection._sourceProject && (
                                            <ReactRouterDOM.Link to={`/${collection._sourceProject.name.toLowerCase()}`} onClick={(e) => e.stopPropagation()} className="text-xs text-on-surface-variant hover:text-primary flex items-center gap-1">
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        <div className="pt-2 pb-4 px-4">
                            <div className="space-y-3">
                                {collection.perks && collection.perks.length > 0 ? (
...
export const ProjectProfileView: React.FC<ProjectProfileViewProps> = ({ project }) => {
        ...
        <div className="space-y-8">
                        ...
                        <div>
                            ...
                            <div className="flex items-center justify-center flex-wrap gap-3 mt-6">
                                <HoverLinkGroup
                                    ...
                                    label="Websites"
                                />
                                {project.twitter_url && <MetricLink href={project.twitter_url} icon={<XSocialIcon />} label="X (Twitter)" metric={project.followers_x} metricLabel="Followers" />}
                                {project.discord_url && <MetricLink href={project.discord_url} icon={<DiscordIcon />} label="Discord" metric={project.members_discord} metricLabel="Members" />}
                                ...
                                <ReactRouterDOM.Link
                                    to={`/${project.name.toLowerCase()}/leaderboard`}
            ...
            <main className="max-w-5xl mx-auto space-y-8 px-4 sm:px-0">
                <Section 
                    ...
                    title="About"
                >
                    <p className="text-base text-on-surface-variant leading-relaxed whitespace-pre-wrap">{project.long_description || project.description}</p>
                </Section>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {project.stage && <div className="neu-card p-4 text-center"><h4 className="font-bold text-sm text-on-surface-variant">Stage</h4><p className="font-bold text-lg text-on-surface">{project.stage}</p></div>}
                    {project.raise_amount && <div className="neu-card p-4 text-center"><h4 className="font-bold text-sm text-on-surface-variant">Raise</h4><p className="font-bold text-lg text-on-surface">{project.raise_amount}</p></div>}
                    {project.drop_status && <div className="neu-card p-4 text-center"><h4 className="font-bold text-sm text-on-surface-variant">Drop Status</h4><p className="font-bold text-lg text-on-surface">{project.drop_status}</p></div>}
                </div>


                {project.strategy_walkthrough && project.strategy_walkthrough.length > 0 && (
                    <StrategyWalkthrough steps={project.strategy_walkthrough} />
                )}
