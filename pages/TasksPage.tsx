                        ...
                        return (
                            <tr key={project.id} className="hover:bg-border/10 transition-colors border-b border-border/10 last:border-b-0">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <button onClick={() => navigate(`/project/${project.id}/tasks`)} className="neu-button px-4 py-1.5 text-sm font-semibold flex items-center gap-1">
...
const TasksPage = () => {
    ...
    useEffect(() => {
        setProjects(getProjects().filter(p => p.tasksInfo && p.tasksInfo.totalTasks > 0));
    }, []);
