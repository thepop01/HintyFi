...
import { getQuestById, submitQuestEntry, voteQuestEntry, submitIdentityAnswer, submitMultipleChoiceAnswer } from '../src/services/dataService';
...
const QuestDetailPage: React.FC = () => {
    ...
    const refreshData = () => {
        if (id) {
            setLoading(true);
            const foundQuest = getQuestById(id);
    ...
    const handleQuestSubmit = (questId: string, twitterLink: string) => {
        if (!currentUser) {
            addToast('Please connect your wallet to submit.', 'info');
            return;
        }
        const result = submitQuestEntry(questId, currentUser.id, twitterLink);
    ...
    const handleIdentityAnswerSubmit = (questId: string, answer: string) => {
        if (!currentUser) {
            addToast('Please connect your wallet to answer.', 'info');
            return;
        }
        const result = submitIdentityAnswer(questId, currentUser.id, answer);
        addToast(result.message, result.success ? 'success' : 'error');
        if (result.success) {
            refreshData();
        }
    ...
    const handleMultipleChoiceAnswerSubmit = (questId: string, answerIndex: number) => {
        if (!currentUser) {
            addToast('Please connect your wallet to answer.', 'info');
            return;
        }
        const result = submitMultipleChoiceAnswer(questId, currentUser.id, answerIndex);
        addToast(result.message, result.success ? 'success' : 'error');
        if (result.success) {
            refreshData();
        }
