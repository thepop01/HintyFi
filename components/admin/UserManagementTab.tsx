...
const UserManagementTab: React.FC = () => {
    const { users, allProjects: projects, refreshData } = useSuperAdminContext();
    ...
    const handleSelectAllProjects = () => {
        setPermissions(prev => {
            ...
            const allSelected = allProjectIds.every(id => prev.associatedProjectIds.includes(id));
            return {
                ...prev,
                associatedProjectIds: allSelected ? [] : allProjectIds
            };
        });
    };
                        ...
                        >
                            ...
                            {userSearchTerm.length > 0 && userSearchSuggestions.length > 0 && (
                                <div>
                                    ...
                                    {userSearchSuggestions.map(u => <button key={u.id} onClick={() => handleUserSearchSelect(u.name)} className="w-full text-left flex items-center gap-3 px-3 py-2 hover:bg-primary/10"><img src={u.profile_pic || `https://i.pravatar.cc/32?u=${u.id}`} alt={u.name} className="w-6 h-6 rounded-full"/><span>{u.name}</span></button>)}
                                </div>
                            )}
                        </motion.div>
                    )}
                            ...
                            <tbody>
                                ...
                                <React.Fragment key={user.id}>
                                    <tr className="border-b border-border/10">
                                        <td className="px-6 py-4"><div className="flex items-center gap-3"><img src={user.profile_pic_url || `https://i.pravatar.cc/32?u=${user.id}`} alt={user.name} className="w-8 h-8 rounded-full" /><span className="font-semibold">{user.name}</span></div></td>
                                    ...
                                    {editingUser?.id === user.id && (
                                        <tr><td colSpan={4} className="p-0 bg-surface/20"><motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="p-4"><div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            {permissions.role === 'project_admin' && (
                                                <div>
                                                    <h4 className="font-bold text-on-surface mb-2">Assign Projects</h4>
                                                    <div className="mb-2">
                                                        <input
                                                            type="text"
                                                    ...
                                                    <div className="max-h-64 overflow-y-auto p-2 rounded-md bg-surface border border-border/20 grid grid-cols-1 sm:grid-cols-2 gap-1">
                                                        <FormCheckbox
                                                            label="Select All Projects"
                                                            checked={filteredProjects.length > 0 && permissions.associatedProjectIds.length === filteredProjects.length}
                                                        ...
                                                        {filteredProjects.map(p =>
                                                            <FormCheckbox
                                                                key={p.id}
                                                                label={p.name}
                                                                checked={permissions.associatedProjectIds.includes(p.id)}
