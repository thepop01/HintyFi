...
const QuestForm: React.FC<QuestFormProps> = ({ questToEdit, questType, onSave, onClose }) => {
    ...
    useEffect(() => {
        if (questToEdit.id) { // Editing existing quest
            let currentTitle = questToEdit.name || '';
            ...
            setCategory(questToEdit.quest_type as any || 'Gaming');
            setStartTime(questToEdit.start_date ? new Date(questToEdit.start_date).getTime() : Date.now());
            setEndTime(questToEdit.end_date ? new Date(questToEdit.end_date).getTime() : Date.now());
            setPointsForSubmission(questToEdit.reward_points || 50);
            setMaxSubmissionsPerUser(questToEdit.max_submissions_per_user || 1);
        }
    ...
    const handleSubmit = () => {
        ...
        if (questType === 'submission') {
            questPayload = {
                ...questPayload,
                quest_type: category,
                reward_points: pointsForSubmission,
