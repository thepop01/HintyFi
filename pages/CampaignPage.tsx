...
import { getEvents, getQuests, getProjects, getQuestById, getSiteContentSettings } from '../src/services/dataService';
...
const CampaignPage: React.FC = () => {
    ...
    const [contentSettings, setContentSettings] = useState<any>({ campaigns: { title: "Where creativity gets rewarded.", subtitle: "Participate in community events and quests to earn recognition and prizes.", featuredProjectIds: [] } });
    ...
    // Filter & Sort State
    const allProjectCategories: ProjectCategory[] = ['defi', 'dex', 'nft', 'gaming', 'social', 'rwa', 'infrastructure', 'wallet', 'depin', 'ai', 'meme', 'launchpad'];
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const settingsData = await getSiteContentSettings();
                setEvents(eventData);
                setQuests(questData);
    ...
    // State for projects needed for campaign filtering
    ...
    const { allCampaignEvents, featuredCampaigns } = useMemo(() => {
        ...
        if (!contentSettings) return { allCampaignEvents: [], featuredCampaigns: [] };
     ...
     }, [events, contentSettings, projects]);
    ...
    const ProjectCampaignGroup: React.FC<ProjectCampaignGroupProps> = ({ project, campaigns, viewType }) => {
        ...
        const { prefetch } = usePrefetch(getQuestById);
    ...
    }, [allCampaignEvents, campaignView, debouncedSearchTerm, activeCategory, sortOption, rewardTypeFilter, allProjects]);
            ...
            <section id="campaigns">
                ...
                {isLoading ? (
                    ...
                    <EmptyState
                        ...
                        description="There are currently no campaigns to display. Please check back later for new and upcoming events."
                    />
                )}
            </section>
        </div>
